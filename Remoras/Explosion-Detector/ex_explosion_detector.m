function ex_explosion_detector(varargin)

parm = varargin{1};

SearchFileMask = {'*df20.x.wav'};
SearchPathMask = parm.baseDir;
SearchRecursiv = parm.recursSearch;

currentPath = mfilename('fullpath');
templateFilePath = fileparts(currentPath);
templateFile = fullfile(templateFilePath,'ex_template.mat');
...
[PathFileList, FileList, PathList] = ...
    ex_utFindFiles(SearchFileMask, SearchPathMask, SearchRecursiv);

templateStruct = load(templateFile);
template = templateStruct.template;
...
for fidx = 1:size(FileList,1)
    ...
    rawStart=[];
    rawDur=[];
    [rawStart,rawDur,fs,rawByteLoc] = ex_readxwavhd(filepath);
